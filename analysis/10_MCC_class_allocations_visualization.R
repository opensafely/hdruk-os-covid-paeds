...
selected_model = load('../Test_run/x.RData')  # Insert path for selected model here
thin = 1
maxi = 50
M0 = selected_model$Mcmc$M0
plotPathsForEta = TRUE

op <- bayesMCClust::calcAllocationsMCC(selected_model,
                                       thin = thin, 
                                       maxi = maxi,
                                       M0 = M0, 
                                       plotPathsForEta = plotPathsForEta)

model_name <- '\\//s*(.*?)s*.RData'
model_name <- regmatches(workspaceFile, regexec(model_name, workspaceFile))[[1]][2]
class_allocation_file_name = paste0('class_allocations_',model_name,'.csv')
...
ggsave(paste0(format(Sys.time(), "%Y%m%d_%H%M%S"),'_plot.png'))
#===============================================================================