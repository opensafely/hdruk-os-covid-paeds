...
def covid_negative_test_date_X(n):
    ...
    for i in range(1, n+1):
        if i == 1:
            variables = var_signature("covid_negative_test_date_1", start_date)
        else:
...
n_negative_test_high = gbl_vars["n_negative_test_high"]
...
study = StudyDefinition(
    ...
    population=patients.satisfying(
        ...
        (NOT has_died)
        AND
        (covid_negative_test_count > 0) AND (covid_negative_test_count < 26)
        """,
